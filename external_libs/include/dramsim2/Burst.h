    ...
    bool fp16Similar(const BurstType& rhs, float epsilon)
    {
        for (int i = 0; i < 16; i++)
        {
            if ((convertH2F(fp16Data_[i]) - convertH2F(rhs.fp16Data_[i])) /
                convertH2F(fp16Data_[i]) > epsilon)
            {
                return false;
            }
        }
    ...
    void loadFp16(string filename)
    {
        ...
        for (int i = 0; i < u16Data.size(); i += 16)
        {
            BurstType burst((u16Data[i]), (u16Data[i + 1]), (u16Data[i + 2]), (u16Data[i + 3]),
                            (u16Data[i + 4]), (u16Data[i + 5]), (u16Data[i + 6]),
                            (u16Data[i + 7]), (u16Data[i + 8]), (u16Data[i + 9]),
                            (u16Data[i + 10]), (u16Data[i + 11]), (u16Data[i + 12]),
                            (u16Data[i + 13]), (u16Data[i + 14]), (u16Data[i + 15]));
            bData.push_back(burst);
        }
    }
