...
class PIMKernel
{
  ...
  public:
    PIMKernel(shared_ptr<MultiChannelMemorySystem> mem, int num_pim_chan, int num_pim_rank) :
              ...
              mode_(PIMConfiguration::getPIMMode()), num_banks_(getConfigParam(UINT, "NUM_BANKS")),
    ...
    */
    ...
    void computeGemv(NumpyBurstType* data, int num_input_tiles, int numOutputTile, int inputTile,
                     int outputTile, int batchIdx, pimBankType bank_types);
