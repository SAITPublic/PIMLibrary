...
{
    DLOG(INFO) << "Launcher for PIM_Lstm";
...
class PimLstmOp : public OpKernel
{
   public:
    explicit PimLstmOp(OpKernelConstruction* context) : OpKernel(context) {}
    ...
    void Compute(OpKernelContext* context) override
    {
        ...
        int ws_len;
        PimCopyMemory((void*)&ws_len, (void*)ws_len_tensor.flat<int32>().data(), sizeof(int), DEVICE_TO_HOST);
...
REGISTER_KERNEL_BUILDER(Name("PimLstm").Device(DEVICE_GPU), PimLstmOp);
