...
from py_fim_ops import py_fim_lstm


class FimLstmFunction(Function):
    @staticmethod
    def forward(ctx, inp, weight, reorder):
        output = py_fim_lstm(inp, weight, reorder)
...
class FimLstm(nn.Module):
    ...
    def forward(self, inp):
        return FimLstmFunction.apply(inp, self.weight, self.reorder)
