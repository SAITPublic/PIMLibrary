...
from py_pim_ops import py_pim_gemv


class PimGemvFunction(Function):
    @staticmethod
    def forward(ctx, inp, weight, reorder):
        output = py_pim_gemv(inp, weight, reorder)
...
class PimGemv(nn.Module):
    ...
    def forward(self, inp):
        return PimGemvFunction.apply(inp, self.weight, self.reorder)
