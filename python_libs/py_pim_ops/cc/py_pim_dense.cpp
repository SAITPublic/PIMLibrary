...
torch::Tensor py_pim_dense(torch::Tensor input, torch::Tensor weights, torch::Tensor bias_flag, torch::Tensor bias){
	...
	if(num_dims==3){
		num_iters = input.sizes()[1];
	        output_tensor = torch::zeros({num_batch, num_iters, num_cols}, torch::dtype(torch::kF16).device(input.device().type(),input.device().index()));
	}
	else{
	        output_tensor = torch::zeros({num_batch, num_cols},torch::dtype(torch::kF16).device(input.device().type(),input.device().index()));
        }
