...
int pim_gemm_bias_relu_4x8x4096_4x4096x1024(bool block)
{
    int ret = 0;
    int n = 1;
    int c = 4;
...
int pim_gemm_profile(int n, int c, int inout_h, int in_w, int out_w)
{
    int ret = 0;
    PimActFunc act = ACT_RELU;
    int in_size = n * c * inout_h * in_w;
...
{
    EXPECT_TRUE(pim_gemm_bias_relu_4x8x4096_4x4096x1024(true) == 0);
}
TEST(HIPIntegrationTest, pim_gemm_profile)
{
    EXPECT_TRUE(pim_gemm_profile(1, 4, 8, 1024, 4096) == 0);
    EXPECT_TRUE(pim_gemm_profile(1, 4, 8, 4096, 1024) == 0);
}
