...
void matmulCPU(half* output, half* input0, half* input1, int m, int n, int k, float alpha, float beta)
{
...
int custom_gemv_test(bool block)
{
    ...
    matmulCPU(output0_h, input0_h, input1_h, 1, out_size, in_size, alpha, beta);
...
TEST(HIPIntegrationTest, CustomGemvTest) { EXPECT_TRUE(custom_gemv_test(true) == 0); }
