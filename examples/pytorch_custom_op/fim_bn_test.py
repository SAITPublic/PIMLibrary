...
class PyFimBNTestRandom(unittest.TestCase):
    def test(self):
        min_val = 0
        max_val = 64
        shape = [2, 64, 1, 1024]
        ...
        result = py_fim_ops.py_fim_bn(torch.from_numpy(np_input), mean, var,
                                      beta, gamma, var_epsilon)
        assert (result.numpy() == np_input).all()


class PyFimBNTestFile(unittest.TestCase):
    def test(self):
        BATCH = 2
        CH = 64
        HEIGHT = 1
        ...
        result = py_fim_ops.py_fim_bn(inp, mean, var, beta, gamma, epsilon)
