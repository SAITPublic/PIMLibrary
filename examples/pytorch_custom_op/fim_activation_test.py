...
class PyFimActTestConstant(unittest.TestCase):
    def test1(self):
        inp = torch.tensor([-3., 5., -13., -4., 9., 0.], dtype=torch.float16)

        result = py_fim_ops.py_fim_activation(inp)
...
class PyFimActTestRandom(unittest.TestCase):
    def test(self):
        batch_size = [1, 10]
        channel = [32, 128, 384]
        height = [3, 8, 73]
                ...
                for h in height:
                    for w in width:
                        ...
                        inp = np.random.uniform(min_val,
                                                ...
                                                size=[b, c, h,
                                                      w]).astype(np.float16)
                        result = py_fim_ops.py_fim_activation(
...
class PyFimActTestFile(unittest.TestCase):
    def test1(self):
        inp = np.fromfile("test_vectors/load/relu/input_256KB.dat",
                          dtype=np.float16)

        result = py_fim_ops.py_fim_activation(torch.from_numpy(inp))
