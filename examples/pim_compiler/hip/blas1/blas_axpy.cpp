...
int pimc_blas_axpy(int w){
    ...
    for (int i = 0; i < w; i++) {
         ((half_float::half*)golden_output->data)[i] = (5 * ((half_float::half*)host_input0->data)[i]) + ((half_float::half*)host_input1->data)[i];
    }
...
TEST(PimCompilerIntegrationTestAxpy, BlasAxpy) { EXPECT_TRUE(pimc_blas_axpy(256) == 0); }
