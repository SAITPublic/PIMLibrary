...
__global__ void mad_test(uint8_t* pim_ctr, uint8_t* pim_data, uint8_t* pim_data2, uint8_t* output, uint8_t* crf_binary,
                         uint8_t* hab_to_pim, uint8_t* pim_to_hab, uint8_t* test_input1)
{
    ...
    /* MAD */
    W_CMD_R(&pim_ctr[(0x3fff02020 & TARGET_MASK) + offset], test_input1 + hipThreadIdx_x * 16);  // write to srf
    B_CMD(1);
