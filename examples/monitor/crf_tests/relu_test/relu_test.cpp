...
__global__ void relu_test(volatile uint8_t* pim_ctr, volatile uint8_t* pim_data, volatile uint8_t* output,
                          ...
                          volatile uint8_t* test_input1, int chan)
{
    ...
    /* relu */
    addr = addr_gen(ch, 0, 0, 0, 0, 0);
    R_CMD(&pim_data[addr + offset]);  // MOV even_bank to grf_A
    B_CMD(1);
    addr = addr_gen(ch, 0, 0, 0, 0, 0);
    ...
    for (int col = 0; col < 2; col++) {
        for (int bg = 0; bg < 4; bg++) {
            for (int ba = 0; ba < 4; ba++) {
                addr_offset = addr_gen(chan, 0, bg, ba, 2, col);
                PrintHalf(output);
            }
        }
    }
