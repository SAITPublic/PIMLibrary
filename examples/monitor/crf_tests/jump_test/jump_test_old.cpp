...
__global__ void jump_test(uint8_t* pim_ctr, uint8_t* pim_data, uint8_t* pim_data2, uint8_t* output, uint8_t* crf_binary,
                          uint8_t* hab_to_pim, uint8_t* pim_to_hab, uint8_t* test_input1)
{
    ...
    /* jump */
    R_CMD(&pim_data[0x0 + offset]);
    B_CMD(1);
    W_CMD(&pim_data2[0x0 + offset]);
    B_CMD(1);
    R_CMD(&pim_data[0x0 + offset]);
    B_CMD(1);
...
int main(int argc, char* argv[])
{
    ...
    ********************************************/
    ...
    crf_bin_h[1] = 0xf0000000e0000803;
    ...
    hipLaunchKernelGGL(jump_test, dim3(blocks), dim3(threadsPerBlock), 0, 0, (uint8_t*)pim_base, (uint8_t*)pim_base,
                       (uint8_t*)pim_base + 0x100000, (uint8_t*)pim_base + 0x200000, (uint8_t*)crf_bin_d,
                       (uint8_t*)mode1_d, (uint8_t*)mode2_d, (uint8_t*)test1_d);

    hipDeviceSynchronize();

    CHECK(hipMemcpy(output_h, (uint8_t*)pim_base + 0x100000, Nbytes, hipMemcpyDeviceToHost));
