...
__global__ void jump_test(uint8_t* fim_ctr, uint8_t* fim_data, uint8_t* fim_out0, uint8_t* fim_out1,
                          uint8_t* crf_binary, uint8_t* hab_to_fim, uint8_t* fim_to_hab, uint8_t* test_input1)
{
    ...
    B_CMD(1);
    addr = addr_gen(ch, 0, 0, 0, 0, 0);
    W_CMD(&fim_out1[addr + offset]);  // NOP
    B_CMD(1);
...
int main(int argc, char* argv[])
{
    ...
    ********************************************/
    ...
    fim_data = fim_base + addr_gen(0, 0, 0, 0, 0, 0);
    fim_out0 = fim_base + addr_gen(0, 0, 0, 0, 1, 0);
    fim_out1 = fim_base + addr_gen(0, 0, 0, 0, 2, 0);

    const unsigned blocks = 1;
    const unsigned threadsPerBlock = 4;

    hipLaunchKernelGGL(jump_test, dim3(blocks), dim3(threadsPerBlock), 0, 0, (uint8_t*)fim_base, (uint8_t*)fim_data,
                       (uint8_t*)fim_out0, (uint8_t*)fim_out1, (uint8_t*)crf_bin_d, (uint8_t*)mode1_d,
