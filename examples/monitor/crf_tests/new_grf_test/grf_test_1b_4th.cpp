                         ...
                         volatile uint8_t* test_input1, volatile uint8_t* test_input2, int chan, int row)
{
    ...
    for (int grf_row = 0; grf_row < NUM_ITER; grf_row++) {
        addr = addr_gen(ch + ch_off, 0, 0, 0, grf_row, 0);
        W_CMD_R(&output[addr + offset], test_input1 + (hipThreadIdx_x % 2) * 16);
#if 0
	...
	B_CMD(1);
        ...
        addr = addr_gen(ch + ch_off, 0, 2, 0, 0x27ff, 0x1f);
        W_CMD(&pim_ctr[addr + offset]);
        B_CMD(1);
        //    start = clock64();
        //    while(clock64() < (start + 10000000));
        addr = addr_gen(ch + ch_off, 0, 2, 1, 0x27ff, 0x1f);
        W_CMD(&pim_ctr[addr + offset]);
        B_CMD(1);
        //    start = clock64();
        //    while(clock64() < (start + 10000000));
        addr = addr_gen(ch + ch_off, 0, 0, 0, 0x27ff, 0x1f);
        W_CMD(&pim_ctr[addr + offset]);
        B_CMD(1);
        //    start = clock64();
        //    while(clock64() < (start + 10000000));
        addr = addr_gen(ch + ch_off, 0, 0, 1, 0x27ff, 0x1f);
        W_CMD(&pim_ctr[addr + offset]);
        B_CMD(1);
        //    start = clock64();
        //    while(clock64() < (start + 10000000));
#endif
#if 1
        addr = addr_gen(ch + ch_off, 0, 0, 0, 0x3fff, 0x8);
        ...
        addr = addr_gen(ch + ch_off, 0, 0, 0, 0x2fff, 0x1f);
        W_CMD(&pim_ctr[addr + offset]);
        B_CMD(1);
        addr = addr_gen(ch + ch_off, 0, 0, 1, 0x2fff, 0x1f);
        W_CMD(&pim_ctr[addr + offset]);
        B_CMD(1);
#endif
        addr = addr_gen(ch + ch_off, 0, 0, 0, 0x3020, park_col);
        R_CMD(&pim_ctr[addr + offset]);
        B_CMD(1);
        B_CMD(0);
#if 0
