...
__device__ inline void R_CMD(volatile uint8_t* addr)
{
    ...
    //    asm volatile("flat_load_dwordx4 v[84:87], %0\n\t" ::"v"(addr) : "v84", "v85", "v86", "v87");
}
...
__device__ inline void W_CMD(volatile uint8_t* addr)
{
    ...
    //    asm volatile("flat_store_dwordx4 %0, v[80:83]\n\t" ::"v"(addr) : "v80", "v81", "v82", "v83");
}
...
__global__ void output_deinit(volatile uint8_t* pim_ctr, volatile uint8_t* pim_data, volatile uint8_t* output,
                              ...
                              volatile uint8_t* test_input1, volatile uint8_t* test_input2, int chan, int row)
{
    ...
    for (int grf_row = 0; grf_row < NUM_ITER; grf_row++) {
        ...
        W_CMD_R(&output[addr + offset], test_input2 + hipThreadIdx_x * 16);
    }
}
                         ...
                         volatile uint8_t* test_input1, volatile uint8_t* test_input2, int chan, int row)
{
    ...
    for (int grf_row = 0; grf_row < NUM_ITER; grf_row++) {
        addr = addr_gen(ch, 0, 0, 0, grf_row, gidx);
        W_CMD_R(&output[addr + offset], test_input1 + w_idx * 16);
        B_CMD(1);
        B_CMD(0);
        ...
        W_CMD_R(&pim_ctr[addr + offset], test_input2 + w_idx * 16);  // write to grf_A
        B_CMD(1);
...
int main(int argc, char* argv[])
{
    ...
    ********************************************/
    ...
    test1_h[0] = 0xffffffffffffffff;
    test1_h[1] = 0xffffffffffffffff;
    test1_h[2] = 0xffffffffffffffff;
    test1_h[3] = 0xffffffffffffffff;
    test2_h[0] = 0xaaaaaaaaaaaaaaa1;
    test2_h[1] = 0xbbbbbbbbbbbbbbb2;
    test2_h[2] = 0xccccccccccccccc3;
    test2_h[3] = 0xddddddddddddddd4;
    test3_h[0] = 0x0000000000000000;
    test3_h[1] = 0x0000000000000000;
    test3_h[2] = 0x0000000000000000;
    test3_h[3] = 0x0000000000000000;
            ...
            for (int bg = 0; bg < 4; bg++) {
                for (int ba = 0; ba < 4; ba += 2) {
                    for (int col = 0; col < 8; col++) {
                        addr_offset = addr_gen(chan, 0, bg, ba, out_row + i, col);
                        output_d = (uint64_t*)((uint8_t*)pim_base + addr_offset);
                        if (output_d[0] != test2_h[0] || output_d[1] != test2_h[1] || output_d[2] != test2_h[2] ||
                            output_d[3] != test2_h[3]) {
                            printf("ch: %d failed idx: %d bg: %d ba: %d col: %d\n", chan, i, bg, ba, col);
                            printf("%#018lx %#018lx %#018lx %#018lx\n", output_d[3], output_d[2], output_d[1],
                                   output_d[0]);
                            failed = 1;
                        }
                        //          else {
                        //              printf("success idx: %d\n", i);
                        //              printf("%#018lx %#018lx %#018lx %#018lx\n", output_d[3], output_d[2],
                        //              output_d[1], output_d[0]);
                    }
                }
            }
            if (failed) {
...
#if 0
    hipDeviceSynchronize();
    hipLaunchKernelGGL(output_deinit, dim3(blocks), dim3(threadsPerBlock), 0, 0, (uint8_t*)pim_base, (uint8_t*)pim_base,
	    (uint8_t*)pim_out, (uint8_t*)crf_bin_d,
	    (uint8_t*)mode1_d, (uint8_t*)mode2_d, (uint8_t*)test1_d, (uint8_t*)test3_d, ch, 1);
    hipDeviceSynchronize();
#endif
