...
__device__ inline void R_CMD(volatile uint8_t* addr)
{
    ...
    asm volatile("flat_load_dwordx4 v[84:87], %0" ::"v"(addr) : "v84", "v85", "v86", "v87");
}
...
__device__ inline void W_CMD(volatile uint8_t* addr)
{
    ...
    asm volatile("flat_store_dwordx4 %0, v[80:83]" ::"v"(addr) : "v80", "v81", "v82", "v83");
}
...
int main(int argc, char* argv[])
{
    ...
    ********************************************/
    ...
    CHECK(output == 0 ? hipErrorOutOfMemory : hipSuccess);
    ...
    for (int i = 0; i < iter; i++) {
        ...
        hipLaunchKernelGGL(grf_test, dim3(blocks), dim3(threadsPerBlock), 0, 0, (uint8_t*)pim_base, (uint8_t*)pim_base,
                           ...
                           (uint8_t*)test1_d, (uint8_t*)test2_d, ch, 1);
        hipDeviceSynchronize();
        ...
        for (int chan = ch; chan < (ch + BLOCKS); chan++) {
            for (int i = 0; i < NUM_ITER; i++) {
                ...
                for (int ba = 0; ba < 4; ba += 2) {
                    for (int bg = 0; bg < 4; bg++) {
                        addr_offset = addr_gen(chan, 0, bg, ba, out_row + i, 0);
                        //	    addr_offset = addr_gen(chan, 0, 0, 0, out_row + i, 0);
                        output = (uint64_t*)((uint8_t*)pim_base + addr_offset);
                        if (output[0] != test2_h[0] || output[1] != test2_h[1] || output[2] != test2_h[2] ||
                            output[3] != test2_h[3]) {
                            printf("ch: %d failed idx: %d bg: %d ba: %d ", chan, i, bg, ba);
