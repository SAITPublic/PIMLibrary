                              ...
                              int num_input_tile)
{
    ...
    if (hipThreadIdx_x < 16) {
        ...
        for (int i_idx = 0; i_idx < num_input_tile /*n_compute_tile*/; i_idx += 2) {
            ...
            addr = addr_gen(ch, 0, 0, 0, row_even, gidx);
            R_CMD(&pim_data[addr + offset]);

            addr = addr_gen(ch, 0, 0, 0, row_even, gidx + 8);
            R_CMD(&pim_data[addr + offset]);

            addr = addr_gen(ch, 0, 0, 0, row_even, gidx + 16);
            R_CMD(&pim_data[addr + offset]);

            addr = addr_gen(ch, 0, 0, 0, row_even, gidx + 24);
            R_CMD(&pim_data[addr + offset]);

            addr = addr_gen(ch, 0, 0, 0, row_odd, gidx);
            R_CMD(&pim_data[addr + offset]);

            addr = addr_gen(ch, 0, 0, 0, row_odd, gidx + 8);
            R_CMD(&pim_data[addr + offset]);

            addr = addr_gen(ch, 0, 0, 0, row_odd, gidx + 16);
            R_CMD(&pim_data[addr + offset]);

            addr = addr_gen(ch, 0, 0, 0, row_odd, gidx + 24);
            R_CMD(&pim_data[addr + offset]);
            B_CMD(1);
            // B_CMD(1);
        }
        for (int i_idx = 1; i_idx < num_input_tile /*n_compute_tile*/; i_idx += 2) {
            ...
            addr = addr_gen(ch, 0, 0, 1, row_even, gidx);
            R_CMD(&pim_data[addr + offset]);

            addr = addr_gen(ch, 0, 0, 1, row_even, gidx + 8);
            R_CMD(&pim_data[addr + offset]);

            addr = addr_gen(ch, 0, 0, 1, row_even, gidx + 16);
            R_CMD(&pim_data[addr + offset]);

            addr = addr_gen(ch, 0, 0, 1, row_even, gidx + 24);
            R_CMD(&pim_data[addr + offset]);

            addr = addr_gen(ch, 0, 0, 1, row_odd, gidx);
            R_CMD(&pim_data[addr + offset]);

            addr = addr_gen(ch, 0, 0, 1, row_odd, gidx + 8);
            R_CMD(&pim_data[addr + offset]);

            addr = addr_gen(ch, 0, 0, 1, row_odd, gidx + 16);
            R_CMD(&pim_data[addr + offset]);

            addr = addr_gen(ch, 0, 0, 1, row_odd, gidx + 24);
            R_CMD(&pim_data[addr + offset]);
            B_CMD(1);
        }
    ...
    for (int i = 0; i < 1 /*batch_dim * num_out_tile*/; i++) {
        if (out_idx < 4096 /*output_dim*/) {
            t_output = 0;
            row = 0;
            col = hipThreadIdx_x % 8 + ((i % 4) * 8);
            addr = addr_gen(ch, 0, bg, ba, row, col);
            for (int j = 0; j < 16; j++) {
                t_output += ((half*)temp_output)[(addr / 2) + j];
            }
            ...
            for (int bg = 0; bg < 4; bg++) {
                for (int ba = 1; ba < 4; ba += 2) {
                    ...
                    for (int col = 0; col < 8; col++) {
                        ...
                        failed = compare_out((uint64_t*)golden_out + (gi++) * 4,
