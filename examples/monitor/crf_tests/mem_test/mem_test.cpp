        ...
        for(int ba = 0; ba < 4; ba++) {
            for (int row = 0; row < 1000; row++) {
                for (int col = 0; col < 32; col++) {
                    ...
                    W_CMD_R(&output[addr + offset], &a + hipThreadIdx_x * 16);
                }
            }
        }
    }
...
int main(int argc, char* argv[])
{
    ...
    ********************************************/
    ...
    test2_h[0] = 0x0000000000000008;
    test2_h[1] = 0x000000000000000c;
    test2_h[2] = 0x0000000000000001;
    test2_h[3] = 0x0000000000000002;
            ...
            for (int ba = 0; ba < 4; ba++) {
                for (int row = 0; row < 100; row++) {
                    for (int col = 0; col < 32; col++) {
                        addr = addr_gen(ch, 0, bg, ba, row, col);
                        test1_h[0] = ch;
                        test1_h[1] = ch * 2 * bg + col;
                        test1_h[2] = ch * 3 * ba + col;
                        test1_h[3] = ch * 4 * row + col;
                        memcpy((uint8_t*)pim_base + addr, test1_h, 32);
                    }
                }
            }
        }
        ...
        for (int bg = 0; bg < 4; bg++) {
            for (int ba = 0; ba < 4; ba++) {
                for (int row = 0; row < 100; row++) {
                    for (int col = 0; col < 32; col++) {
                        addr = addr_gen(ch, 0, bg, ba, row, col);
                        uint64_t a = ((uint64_t*)((uint8_t*)pim_base + addr))[0];
                        uint64_t b = ((uint64_t*)((uint8_t*)pim_base + addr))[1];
                        uint64_t c = ((uint64_t*)((uint8_t*)pim_base + addr))[2];
                        uint64_t d = ((uint64_t*)((uint8_t*)pim_base + addr))[3];
                        if (a != ch || b != ch * 2 * bg + col || c != ch * 3 * ba + col || d != ch * 4 * row + col) {
                            printf("ch: %d bg: %d ba: %d row: %d col: %d %lx%lx%lx%lx\n", ch, bg, ba, row, col, a, b, c,
                                   d);
                        }
                    }
                }
    ...
    for (int i = 0; i < 100; i++) {
        ...
        hipLaunchKernelGGL(dummy_print, dim3(blocks), dim3(threadsPerBlock), 0, 0);
        hipDeviceSynchronize();
        hipLaunchKernelGGL(grf_test, dim3(blocks), dim3(threadsPerBlock), 0, 0, (uint8_t*)pim_base, (uint8_t*)pim_base,
                           ...
                           (uint8_t*)mode1_d, (uint8_t*)mode2_d, (uint8_t*)test1_d, (uint8_t*)test2_d, chan);
        ...
        addr_offset = addr_gen(chan, 0, 0, 2, out_row, 0);
        output = (uint64_t*)((uint8_t*)pim_base + addr_offset);
        if (output[0] != test2_h[0] || output[1] != test2_h[1] || output[2] != test2_h[2] || output[3] != test2_h[3]) {
            printf("failed idx: %d\n", i);
            printf("%#018lx %#018lx %#018lx %#018lx\n", output[3], output[2], output[1], output[0]);
        ...
        addr_offset = addr_gen(chan, 0, 1, 0, out_row, 0);
        output = (uint64_t*)((uint8_t*)pim_base + addr_offset);
        if (output[0] != test2_h[0] || output[1] != test2_h[1] || output[2] != test2_h[2] || output[3] != test2_h[3]) {
            printf("failed idx: %d\n", i);
            printf("%#018lx %#018lx %#018lx %#018lx\n", output[3], output[2], output[1], output[0]);
        ...
        addr_offset = addr_gen(chan, 0, 1, 2, out_row, 0);
        output = (uint64_t*)((uint8_t*)pim_base + addr_offset);
        if (output[0] != test2_h[0] || output[1] != test2_h[1] || output[2] != test2_h[2] || output[3] != test2_h[3]) {
            printf("failed idx: %d\n", i);
            printf("%#018lx %#018lx %#018lx %#018lx\n", output[3], output[2], output[1], output[0]);
        ...
        addr_offset = addr_gen(chan, 0, 2, 0, out_row, 0);
        output = (uint64_t*)((uint8_t*)pim_base + addr_offset);
        if (output[0] != test2_h[0] || output[1] != test2_h[1] || output[2] != test2_h[2] || output[3] != test2_h[3]) {
            printf("failed idx: %d\n", i);
            printf("%#018lx %#018lx %#018lx %#018lx\n", output[3], output[2], output[1], output[0]);
        ...
        addr_offset = addr_gen(chan, 0, 2, 2, out_row, 0);
        output = (uint64_t*)((uint8_t*)pim_base + addr_offset);
        if (output[0] != test2_h[0] || output[1] != test2_h[1] || output[2] != test2_h[2] || output[3] != test2_h[3]) {
            printf("failed idx: %d\n", i);
            printf("%#018lx %#018lx %#018lx %#018lx\n", output[3], output[2], output[1], output[0]);
        ...
        addr_offset = addr_gen(chan, 0, 3, 0, out_row, 0);
        output = (uint64_t*)((uint8_t*)pim_base + addr_offset);
        if (output[0] != test2_h[0] || output[1] != test2_h[1] || output[2] != test2_h[2] || output[3] != test2_h[3]) {
            printf("failed idx: %d\n", i);
            printf("%#018lx %#018lx %#018lx %#018lx\n", output[3], output[2], output[1], output[0]);
        ...
        addr_offset = addr_gen(chan, 0, 3, 2, out_row, 0);
        output = (uint64_t*)((uint8_t*)pim_base + addr_offset);
        if (output[0] != test2_h[0] || output[1] != test2_h[1] || output[2] != test2_h[2] || output[3] != test2_h[3]) {
            printf("failed idx: %d\n", i);
            printf("%#018lx %#018lx %#018lx %#018lx\n", output[3], output[2], output[1], output[0]);
