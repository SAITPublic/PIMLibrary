...
__global__ void mul_test(volatile uint8_t* fim_ctr, volatile uint8_t* fim_data0, volatile uint8_t* fim_data1, volatile uint8_t* output,
                         volatile uint8_t* crf_binary, volatile uint8_t* hab_to_fim, volatile uint8_t* fim_to_hab, volatile uint8_t* test_input1)
{
    ...
    B_CMD(1);
    addr = addr_gen(ch, 0, 0, 0, 0, 0);
    R_CMD(&fim_data1[addr + offset]); // MUL grf_A, EVEN_BANK
    B_CMD(1);
    addr = addr_gen(ch, 0, 0, 0, 0, 0);
...
int main(int argc, char* argv[])
{
    ...
    ********************************************/
    ...
    crf_bin_h[2] = 0xf000000000000005;
    ...
    fim_data0 = fim_base + addr_gen(0, 0, 0, 0, 0, 0);
    fim_data1 = fim_base + addr_gen(0, 0, 0, 0, 1, 0);
    ...
    for (int i = 0; i < 16; i++) {
        ((half_float::half*)fim_data0)[(addr_offset >> 1) + i] = half_float::half(1.0);
        ((half_float::half*)fim_data1)[(addr_offset >> 1) + i] = half_float::half(3.0);
    }

    const unsigned blocks = 1;
    const unsigned threadsPerBlock = 4;

    hipLaunchKernelGGL(mul_test, dim3(blocks), dim3(threadsPerBlock), 0, 0, (uint8_t*)fim_base, (uint8_t*)fim_data0,
