...
__global__ void mul_test(volatile uint8_t* pim_ctr, volatile uint8_t* pim_data0, volatile uint8_t* pim_data1,
                         volatile uint8_t* output, volatile uint8_t* crf_binary, volatile uint8_t* hab_to_pim,
                         volatile uint8_t* pim_to_hab, volatile uint8_t* test_input1, int chan)
{
    ...
    B_CMD(1);
    addr = addr_gen(ch, 0, 0, 0, 0, gidx);
    R_CMD(&pim_data1[addr + offset]);  // MUL grf_A, EVEN_BANK
    B_CMD(1);
    addr = addr_gen(ch, 0, 0, 0, 0, gidx);
...
int main(int argc, char* argv[])
{
    ...
    ********************************************/
    ...
    crf_bin_h[1] = 0x2910011029100000;
    ...
    pim_data0 = pim_base + addr_gen(chan, 0, 0, 0, 0, 0);
    pim_data1 = pim_base + addr_gen(chan, 0, 0, 0, 1, 0);
    pim_out = pim_base + addr_gen(chan, 0, 0, 0, 2, 0);

    uint64_t addr_offset;

    addr_offset = addr_gen(chan, 0, 0, 0, 0, 0);
    for (int i = 0; i < 16; i++) {
        ((half_float::half*)pim_data0)[(addr_offset >> 1) + i] = half_float::half(2.0);
        ((half_float::half*)pim_data1)[(addr_offset >> 1) + i] = half_float::half(3.0);
    }

    const unsigned blocks = 1;
    const unsigned threadsPerBlock = 4;

    hipLaunchKernelGGL(mul_test, dim3(blocks), dim3(threadsPerBlock), 0, 0, (uint8_t*)pim_base, (uint8_t*)pim_data0,
                       (uint8_t*)pim_data1, (uint8_t*)pim_out, (uint8_t*)crf_bin_d, (uint8_t*)mode1_d,
                       (uint8_t*)mode2_d, (uint8_t*)test1_d, chan);
    ...
    addr_offset = addr_gen(chan, 0, 0, 0, 2, 1);
    output = (uint64_t*)((uint8_t*)pim_base + addr_offset);
    PrintHalf(output);
    addr_offset = addr_gen(chan, 0, 0, 2, 2, 1);
    output = (uint64_t*)((uint8_t*)pim_base + addr_offset);
    PrintHalf(output);
    addr_offset = addr_gen(chan, 0, 1, 0, 2, 1);
    output = (uint64_t*)((uint8_t*)pim_base + addr_offset);
    PrintHalf(output);
    addr_offset = addr_gen(chan, 0, 1, 2, 2, 1);
    output = (uint64_t*)((uint8_t*)pim_base + addr_offset);
    PrintHalf(output);
    addr_offset = addr_gen(chan, 0, 2, 0, 2, 1);
    output = (uint64_t*)((uint8_t*)pim_base + addr_offset);
    PrintHalf(output);
    addr_offset = addr_gen(chan, 0, 2, 2, 2, 1);
    output = (uint64_t*)((uint8_t*)pim_base + addr_offset);
    PrintHalf(output);
    addr_offset = addr_gen(chan, 0, 3, 0, 2, 1);
    output = (uint64_t*)((uint8_t*)pim_base + addr_offset);
    PrintHalf(output);
    addr_offset = addr_gen(chan, 0, 3, 2, 2, 1);
    output = (uint64_t*)((uint8_t*)pim_base + addr_offset);
    PrintHalf(output);
#endif
