...
int PimMemoryManager::convert_data_layout(PimBo* dst, PimBo* src0, PimBo* src1, PimOpType op_type)
{
    DLOG(INFO) << "[START] " << __FUNCTION__ << " called";
    int ret = 0;

    if (op_type == OP_ELT_ADD || op_type == OP_ELT_MUL) {
        ret = convert_data_layout_for_elt_op(dst, src0, PimBankType::EVEN_BANK);
        ret = convert_data_layout_for_elt_op(dst, src1, PimBankType::ODD_BANK);
    } else {
        DLOG(ERROR) << "not yet implemented";
    }
...
int PimMemoryManager::convert_data_layout_for_bn(PimBo* dst, PimBo* src)
{
    DLOG(INFO) << "[START] " << __FUNCTION__ << " called";
    int ret = 0;
    ...
    /* TODO: calculate both start row and col address from PimBo */
    unsigned s_row = 0;
    unsigned s_col = 0;
...
int PimMemoryManager::convert_data_layout_for_relu(PimBo* dst, PimBo* src)
{
    DLOG(INFO) << "[START] " << __FUNCTION__ << " called";
    int ret = 0;
    ...
    uint32_t s_col = 0;
    uint32_t trans_size = fbi_.trans_size;
    uint32_t type_size = (precision_ == PIM_FP16) ? sizeof(half) : sizeof(char);
...
int PimMemoryManager::convert_data_layout_for_elt_op(PimBo* dst, PimBo* src, PimBankType pim_bank_type)
{
    DLOG(INFO) << "[START] " << __FUNCTION__ << " called";
    int ret = 0;
    ...
    for (int x = 0; x < dim_operand; x += num_grf) {
        ...
        for (int grf_idx = 0; grf_idx < num_grf; grf_idx++) {
            addr_op = addr_gen_safe(cidx, rank, bg, bank + (int)pim_bank_type, row, col);
            memcpy(dst_data + addr_op, src_data + (x + grf_idx) * trans_size, trans_size);
            col++;
        }
