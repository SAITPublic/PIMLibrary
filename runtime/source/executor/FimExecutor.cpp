...
namespace executor
{
FimExecutor::FimExecutor(FimRuntimeType rt_type, FimPrecision precision) : rt_type_(rt_type), precision_(precision)
{
                            ...
                            double epsilon, hipStream_t stream, bool block)
{
    ...
    fim_emulator_->execute_bn(output, fim_data, h_fmtd32_, h_fmtd32_size_[0], g_fim_base_addr, fim_gemv_tmp_buffer_);
#else
    if (block) hipStreamSynchronize(stream);
#endif
    delete[] srf_binary;
